...
public class bj_2456 {
	...
	public static void main(String[] args) {
		...
		int [] tsum = new int[3]; //임시 총계
		int arr [][] = new int [4][3]; //후보 선호도 저장 
		int num=0;
		...
		for(int i=0;i<N;i++) {
			for(int j=0;j<3;j++) {
				num = s.nextInt();
				arr[num-1][j] ++; //3,2,1 점 별 개수
				tsum[j] +=num; //후보 총계
			}
		}
		
		//점수별 가중치 합계 1점*1 , 2점 *2, 3점 *3
		for(int i=0;i<3;i++) {
			arr[3][i] = arr[0][i] +arr[1][i]*2 +arr[2][i]*3;
		}
		
		int maxnum=0,k=0;
		if(arr[3][0] ==arr[3][1] &&arr[3][1] == arr[3][2]) {
			maxnum =tsum[0];
		}else {
			for(int i=0;i<3;i++) {
				if (maxnum<arr[3][i]) {
