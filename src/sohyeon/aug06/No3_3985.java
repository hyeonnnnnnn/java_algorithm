...
이때, 이미 번호가 적혀있는 조각은 번호를 적지 못하고 넘어간다. 이런 방식을 이용해서 방청객에게 조각을 주다보니, 자신이 원래 원했던 조각을 받지 못하는 경우가 생길 수 있다.
...
public class No3_3985 {
	...
	public static void main(String[] args) {
		...
		int num1 = 0;															// 가장 많은 조각을 받을 것으로 기대하고 있던 방청객
		int num2 = 0;															// 실제로 가장 많은 조각을 받은 방청객
		int max = 0;															// 최대 조각 수
		
		int maxSum = Integer.MIN_VALUE;
		int cake[] = new int[L];												// 롤케이크
		
		for (int i = 0; i < N; i++) {
			int P = sc.nextInt();												// P번 조각
			int K = sc.nextInt();												// K번 조각
			int sum = 0;														// 조각 수
			
			// 예상
			if ((K - P) > max) {
				max = (K - P);													// 최대로 받을 수 있는 예상 조각 수
				num1 = i + 1;													// 가장 많은 조각을 받을 것으로 예상되는 방청객
			}
			...
			for (int j = P - 1; j < K; j++) {
				if (cake[j] == 0) {												// 비어있으면 현재 방청객 번호 넣어주기
					cake[j] = (i + 1);
					sum++;														// 방청객이 가지고 있는 실제 조각 수++
				} 
			}
			if (sum > maxSum) {													// 실제 조각 수 비교해서
				num2 = (i + 1);													// 실제 가장 많은 조각 가지고 있는 방청객
				maxSum = sum;
			}
		}
		sc.close();
