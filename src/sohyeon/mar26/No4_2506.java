...
public class No4_2506 {
	...
	public static void main(String[] args) {
		...
		int arr[] = new int[N];											// 채점 결과
		int score[] = new int[N];										// 점수
		int num = 0;													// 누적 점수
		int result = 0;													// 최종 점수
		
		for(int i = 0; i < N; i++) {									// 전체 채점결과 배열에 저장
			arr[i] = sc.nextInt();
		}
		
		for(int i = 0; i < N; i++) {
			if(arr[i] == 0) {											// 채점결과 0일 때 점수 0 배열에 저장
				num = 0;
				score[i] = num;
			} else if(arr[i] == 1) {									// 채점결과 1일 때 점수 1 증가시켜서 배열에 저장. 연속으로 1일 경우 기존 점수에서 +1 되어 누적
				num++;
				score[i] = num;
			}
		}
		
		for(int i = 0; i < N; i++) {									// 총 점수 계산
			result += score[i];
		}
